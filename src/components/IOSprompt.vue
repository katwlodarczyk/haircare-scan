<template>
  <div class="w-screen h-screen fixed flex flex-col z-40 top-0 right-0 left-0">
    <div
      class="absolute w-screen h-screen bg-gray-700 opacity-75 transition-opacity"
    />
    <div
      class="iosPrompt bg-[#eee] rounded-md flex flex-col py-2 z-40 font-sans mx-4 fixed bottom-0"
      :class="isVisible ? 'visible' : ''"
    >
      <div class="flex flex-row justify-between border-b pb-2 border-gray-200">
        <h3 class="pl-4 text-gray-700 text-base">Add to Home Screen</h3>
        <p @click="$emit('closePrompt')" class="pr-4 text-base text-[#49f]">
          Cancel
        </p>
      </div>
      <p
        class="text-sm text-left px-6 py-4 text-gray-500 border-b border-gray-200"
      >
        This website has app functionality. Add it to your home screen to use it
        in fullscreen and while offline.
      </p>
      <div
        class="flex flex-col space-y-4 px-6 py-3 text-gray-600 font-medium text-sm text-left"
      >
        <div class="flex flex-row space-x-6 items-center">
          <svg
            class="w-7 h-7 text-[#49f]"
          ...
          <p>1) Press the 'Share' button on the menu bar below.</p>
        </div>
        <div class="flex flex-row space-x-6 items-center">
          <svg
            class="w-7 h-7"
